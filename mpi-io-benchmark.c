...
int main(int argc, char **argv)
{
...
#if NVME
  sprintf(path, "/mnt/nvme/uid_%s/job_%s/%s%d.txt", getenv("SLURM_JOB_UID"), getenv("SLURM_JOB_ID"), "BlockTest", BLOCKSIZE);
#else
  getcwd(path, 511);
  ...
  MPI_File_open(MPI_COMM_WORLD, path, MPI_MODE_RDWR | MPI_MODE_CREATE, MPI_INFO_NULL, &fh);
  for (int i = 0; i < 32; i++)
  {
    bufsize = BLOCKSIZE / totalRanks;
